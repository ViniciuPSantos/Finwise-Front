...
const field =
  "w-full rounded-sm px-3.5 py-2.5 text-sm bg-white text-ink-900 ring-1 ring-line-strong outline-none focus:ring-1.5 focus:ring-pine-600 transition";
    ...
    <div className="fixed inset-0 bg-[rgba(10,20,16,0.5)] flex items-center justify-center p-5 z-50" onClick={onClose}>
      <div className="bg-surface rounded-lg shadow-pop p-6 w-full max-w-[440px]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display font-bold text-xl text-ink-900">{isEdit ? "Editar" : "Nova"} transação</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
          ...
          <div className="flex gap-1 bg-paper-sunk rounded-sm p-1">
            <button
              ...
              className={`flex-1 py-2 rounded-xs text-[13px] font-semibold transition ${
                type === "EXPENSE" ? "bg-expense text-white" : "text-ink-500"
              }`}
            >
              Despesa
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-xs text-[13px] font-semibold transition ${
                type === "INCOME" ? "bg-income text-white" : "text-ink-500"
              }`}
            >
              Receita
            </button>
          ...
          <div>
            ...
            <input className={field} placeholder="Ex.: Mercado" autoFocus {...register("description")} />
